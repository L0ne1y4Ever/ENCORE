...
| `TBD` | feat | Added Dashboard WebSocket refresh events and connected the admin dashboard to automatic metric reloads. | `mvn test` passed; `npm run build` passed; real STOMP/API emitted `ORDER_PAID` and `ORDER_REFUNDED`; browser dashboard API calls increased from 1 to 3 after live events. | Pending push |
