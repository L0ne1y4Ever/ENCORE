...
| `e3800c3` | feat | Added backend seat locking, order creation, mock payment, ticket generation, and frontend seat/order API integration. | `mvn test` passed; `npm run build` passed; backend lock/order/pay API flow passed; repeated order creation reused the pending order. | Pending push |
