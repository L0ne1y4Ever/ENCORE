...
| current loop | feat | Added backend-ranked Top 8 recommendations and a homepage recommendation rail above category tabs. | `mvn -Dtest=ShowServiceTest test` passed; `mvn test` passed; `npm run build` passed; real API returned ranked recommendations and browser verification opened `/show/s-001` from a recommendation card. | Pending push |
