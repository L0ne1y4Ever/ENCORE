...
| `TBD` | feat | Wired admin schedule/order pages to backend APIs and added admin schedule status, refund, and force-check-in operations. | `mvn test` passed; `npm run build` passed; real admin API schedule update/restore, force check-in, and refund flow passed. | Pending |
