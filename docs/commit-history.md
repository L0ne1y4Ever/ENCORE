...
| `dd161b6` | feat | Bound the check-in scanner to a selected current schedule and rejected tickets from other schedules. | `mvn test` passed; `npm run build` passed; real API rejected a wrong-schedule ticket and accepted the same ticket for the correct schedule; browser verification confirmed schedule persistence and request payload binding. | Pushed to `origin/main` |
| `91e5a0d` | feat | Added a Three.js stage-seat preview above the user seat map while preserving the existing 2D checkout flow. | `npm run build` passed; `mvn test` passed; browser checks verified desktop/mobile nonblank canvas pixels, drag movement, 3D click selection, lock-to-confirm flow, and realtime redraw after a WebSocket lock event. | Pushed to `origin/main` |
...
| current loop | feat | Added Redis-backed group-seat invitations with member seat claims and host checkout. | `mvn -Dtest=GroupOrderServiceTest test` passed; `mvn test` passed; `npm run build` passed; real API and two-browser UI verification completed group invite, friend join, host checkout, payment, and e-ticket flow. | Pending push |
